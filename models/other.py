...
class ZakatCalculation(Base):
    ...
    gold_rate_per_gram     = Column(Numeric(10, 2))
    silver_rate_per_gram   = Column(Numeric(10, 2))
    usd_to_pkr_rate        = Column(Numeric(10, 4))
    ...
    created_at             = Column(DateTime(timezone=True), server_default=func.now())
    # ── New columns (all nullable / defaulted — existing rows unaffected) ──────
    madhab_used              = Column(String(20),   nullable=True)
    nisab_preference_used    = Column(String(20),   nullable=True)
    business_tradeable_pkr   = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    business_cash_pkr        = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    bad_debts_pkr            = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    personal_loans_pkr       = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    credit_card_pkr          = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    car_loan_installments    = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    home_loan_pkr            = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    home_loan_included       = Column(Boolean,         nullable=True, server_default="false")
    other_liabilities_pkr    = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    total_liabilities_pkr    = Column(Numeric(12, 2), nullable=True, server_default="0.00")
    net_zakatable_pkr        = Column(Numeric(12, 2), nullable=True)
    wallet_balance_snapshot  = Column(Numeric(12, 2), nullable=True)
